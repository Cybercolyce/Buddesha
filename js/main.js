...
function encrypt() {
	...
	if (msg.length < 1) {
		$("#error-alert").show();
		$("#copy-alert").hide();
		$("#error-alert").text("无言者，纵真神再临，亦不可渡:");
	} else {
		if (key.length < 1) {
			key = password;
		}
...
function decrypt() {
	...
	if (msg.length < 1) {
		...
		if (msg.substring(0, 3) != "佛曰：") {
			$("#error-alert").show();
			$("#copy-alert").hide();
			$("#error-alert").text("施主可曾记得此为何高僧所言？");
		} else {
			if (key.length < 1) {
				key = password;
			}
...
function togod(msg, key) {
	var str = CryptoJS.DES.encrypt(msg, key).toString();
...
function toman(msg, key) {
	...
	var st = CryptoJS.DES.decrypt("U2FsdGVkX1" + str, key).toString(CryptoJS.enc.Utf8);
